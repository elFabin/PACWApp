...
function bodyContent({ body = '', connected = false }) {
  ...
  <h1>PACWApp</h1>
  ${connected ? '<form hx-post="/disconnect" hx-target="body" hx-swap="innerHTML"><button type="submit" class="btn-secondary">Disconnect</button></form>' : ''}
</header>
<main id="content">
${body}
</main>`;
...
function errorFragment(message, status) {
  ...
  return `<div class="error">Error${statusLabel}: ${escapeHtml(message)}</div>`;
}
...
<section class="grid">
  ...
  <div class="panel panel-wide">
    <h3><button type="button" class="disclosure" onclick="var p=document.getElementById('game-data-panel'); if (!p.dataset.loaded) { p.dataset.loaded='1'; htmx.trigger(p, 'reveal'); } p.classList.toggle('hidden');">Game Data ▾</button></h3>
...
function infoPanel(info) {
  return `<h3>Server Info</h3>
<dl class="kv">
  <dt>Name</dt><dd>${escapeHtml(info.servername)}</dd>
  <dt>Version</dt><dd>${escapeHtml(info.version)}</dd>
  <dt>World GUID</dt><dd>${escapeHtml(info.worldguid)}</dd>
</dl>`;
}
...
<dl class="kv">
  <dt>FPS</dt><dd>${escapeHtml(metrics.serverfps)}</dd>
  <dt>Players</dt><dd>${escapeHtml(metrics.currentplayernum)}/${escapeHtml(metrics.maxplayernum)}</dd>
  <dt>Frame time</dt><dd>${escapeHtml(metrics.serverframetime)} ms</dd>
  <dt>Uptime</dt><dd>${escapeHtml(metrics.uptime)}s</dd>
</dl>`;
}
...
function playersPanel(players) {
  ...
  const rows = list.length
    ...
    <td>${escapeHtml(p.playerId ?? p.userId ?? p.userid)}</td>
    <td>${escapeHtml(p.level)}</td>
    <td class="row-actions">
      <form hx-post="/ui/kick" hx-target="#players-panel" hx-swap="innerHTML" hx-confirm="Kick ${escapeHtml(p.name)}?">
        <input type="hidden" name="userid" value="${escapeHtml(p.userId ?? p.userid)}">
        <button type="submit" class="btn-warn">Kick</button>
      </form>
      <form hx-post="/ui/ban" hx-target="#players-panel" hx-swap="innerHTML" hx-confirm="Ban ${escapeHtml(p.name)}?">
        <input type="hidden" name="userid" value="${escapeHtml(p.userId ?? p.userid)}">
        ...
        .join('\n')
    : `<tr><td colspan="4">No players connected.</td></tr>`;

  return `<h3>Players (${list.length})</h3>
<table class="players-table">
  <thead><tr><th>Name</th><th>User ID</th><th>Level</th><th></th></tr></thead>
  <tbody>${rows}</tbody>
</table>`;
}
